...
│   │   ├── components/
│   │   │   ├── quiz/                ← Quiz Engine
│   │   │   │   ├── QuizEngine.tsx   ← Controller đọc JSON → render
│   │   │   │   ├── MultipleChoice.tsx
│   │   │   │   ├── DragDrop.tsx     ← dùng dnd-kit
│   │   │   │   ├── FillBlank.tsx
│   │   │   │   └── Matching.tsx
│   │   │   ├── dashboard/           ← Trang chủ bé
│   │   │   ├── dream-box/           ← Hộp Quà Ước Mơ
│   │   │   └── ui/                  ← Shared: Button, Card, Avatar...
│   │   ├── pages/                   ← LoginPage, DashboardPage...
│   │   ├── hooks/                   ← useAuth, useQuiz, useDragDrop...
│   │   └── styles/
...
│       ├── plan.md                  ← Tổng quan 11 phases
│       ├── phase-01-foundation.md
│       └── ...phase-11-polish.md
├── CONTRIBUTING.md                  ← Hướng dẫn cộng tác
├── README.md
└── wrangler.json
```
...
```json
{
  ...
  "audio_url": "optional",
  "instructions_vi": "Hướng dẫn bằng tiếng Việt cho bé",
  "questions": [
    {
      "id": "q1",
      "prompt": "Câu hỏi",
...
```
users               ← Phụ huynh đăng ký bằng Google
student_profiles    ← Hồ sơ bé (tên, avatar, theme màu)
student_stats       ← Thống kê (sao, streak, skill levels)
...
### Quy tắc sao
- **Sao KHÔNG bao giờ hết hạn** và KHÔNG bao giờ bị trừ
- Sao dùng để tiến đến mục tiêu trong Hộp Quà Ước Mơ
...
|-------|-----|-----------|-------------|
| **01** | Foundation — D1 Schema + Wrangler | ⬜ Chưa làm | `src/worker/db/` |
| **02** | Theme System + Design Tokens | ⬜ Chưa làm | `src/react-app/styles/` |
| **03** | Auth — Google OAuth | ⬜ Chưa làm | `src/worker/routes/auth.ts` |
| **04** | Quiz Engine — MC + FillBlank | ⬜ Chưa làm | `components/quiz/` |
...
| Quyết định | Chi tiết |
|-----------|---------|
| Stack | Vite + React + Hono + Cloudflare (không đổi Next.js) |
| Database | D1 + R2 hybrid, không cần VPS |
| Theme | 6 themes, CSS variables, đổi instant |
| Dream Box | Bé chọn → Email PH → PH confirm |
| Sao | Không hết hạn, không bao giờ bị trừ |
| Avatar | 12–20 preset SVG, miễn phí hết |
| Thanh toán | Thủ công trong MVP |
...
→ Phase 06 → Phase 07 → Phase 08 → Phase 09 → Phase 10 → Phase 11
```

### Lệnh bắt đầu
```bash
# Đọc phase file trước
cat plans/260428-1029-luyen-thi-io-mvp/phase-01-foundation.md

# Bắt đầu với wrangler D1
wrangler d1 create luyen-thi-db
...
> *"Đọc `docs/CONTEXT.md`, `docs/RULES.md` và `plans/.../phase-01-foundation.md` trước khi bắt đầu."*
